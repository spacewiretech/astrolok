...
/// back; whether the money actually moved is a question only the server can answer, which is why
...
class SdkCashfreeCheckout implements CashfreeCheckout {
  ...
  /// `#RRGGBB`, which is the only form Cashfree's theme builder accepts.
  ...
  Future<CheckoutResult> open({
    ...
    return _run(() {
      // Cashfree's builder takes hex strings, not Colors, so the palette is converted rather
      // than duplicated — a Track B edit to AppColors reskins this screen too.
      final theme = CFThemeBuilder()
          .setNavigationBarBackgroundColorColor(_hex(AppColors.brand))
          .setNavigationBarTextColor('#FFFFFF')
          .setPrimaryTextColor(_hex(AppColors.heading))
          .build();
