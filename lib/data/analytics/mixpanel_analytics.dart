...
class MixpanelAnalytics implements Analytics {
  ...
  static Map<String, Object> peoplePropertiesFor(AppUser user) {
    return <String, Object>{
      ...
      P.inTrial: user.inTrial,
      P.hasEverSubscribed: user.hasEverSubscribed,
      // The birth date is the app's whole premise — every reading and every chat turn is
      // conditioned on it — so whether a profile has one is the first thing any cohort splits on.
      // The year alone rather than the date: it is what makes an age breakdown possible, and the
      // full date is a stronger identifier than anything else here without answering a question
      // the year does not already answer.
      P.hasBirthDate: user.hasBirthDate,
      if (user.birthDate != null) P.birthYear: user.birthDate!.year,
