...
///
/// The splash gate only runs at cold start, which is not enough: the trial is a single day, so
/// it will routinely expire while the app is open in someone's pocket. Without this, a user
/// could keep reading indefinitely simply by never closing the app.
///
...
/// configured grace window, so a user whose ₹249 is mid-settlement is not thrown out by a
...
class _EntitlementGateState extends ConsumerState<EntitlementGate>
  ...
  /// it, walking between two gated screens would spend a round trip answering a question
  ...
  void _armExpiryTimer(DateTime? expiresAt) {
    ...
    _expiryTimer = Timer(until < _maxTimerHorizon ? until : _maxTimerHorizon, _refresh);
  }
