...
class SiteSection extends StatelessWidget {
  ...
  /// Used by the hero, which carries the app's warm cream gradient rather than a flat fill.
...
/// Every page's frame: nav on top, page content and footer scrolling beneath.
...
const _sectionLinks = <({String label, String anchor})>[
  (label: 'Readings', anchor: 'features'),
      ...
      SizedBox(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(SiteCopy.tagline, style: SiteText.footerLink.copyWith(height: 1.6)),
          ],
        ),
      ),
      _FooterColumn(
    ...
    return Container(
      ...
      child: ContentWidth(
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Wrap(spacing: 56, runSpacing: 40, children: columns),
            ...
            Text(
              'Readings are for guidance and reflection, not professional advice.',
              style: SiteText.footerLink.copyWith(fontSize: 13.5, height: 1.6),
            ),
          ],
        ),
...
///
/// `wordmark.png` is navy-and-gold artwork made for a cream ground; on the ink band the navy
/// half of it disappears, so the footer draws the mark instead. Same two tones as [Wordmark],
/// with the navy lifted to white.
class _FooterWordmark extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Text.rich(
      TextSpan(
        children: [
          TextSpan(text: 'Astro', style: base.copyWith(color: Colors.white)),
          TextSpan(text: 'lok', style: base.copyWith(color: SiteColors.brandOnInk)),
        ],
      ),
