...
/// The site's button. [onPressed] null renders the muted, non-interactive state used by the
...
class SiteButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (filled) {
      // Gold is the call to action on both grounds — it is a light hue, so it reads on ink as
      // well as on cream, and swapping to white on dark would lose the brand at the one moment
      // the page is asking for the tap.
      background = enabled ? AppColors.gold : (onDark ? Colors.white24 : AppColors.goldWash);
      foreground = enabled ? AppColors.navy : (onDark ? Colors.white70 : AppColors.muted);
      border = null;
    } else {
...
/// One card in the feature grid. Uses the reading thumbnails already bundled for Home's
/// Explore Readings rows, falling back to the Material glyph named in [FeatureCopy.icon].
        ...
        children: [
          Container(
            ...
            child: SafeImage(
              ...
              height: 64,
              fit: BoxFit.cover,
              fallback: Icon(feature.icon, size: 26, color: AppColors.goldDeep),
            ),
          ),
          const SizedBox(height: 20),
          Row(
            children: [
              Flexible(child: Text(feature.title, style: SiteText.cardTitle(context))),
...
/// Marks a feature the app does not ship yet, matching Home's "coming soon" snackbar.
class _SoonChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(
        'Coming soon',
        style: SiteText.eyebrow.copyWith(fontSize: 11, letterSpacing: 0.2),
      ),
    );
  }
}
    ...
    return Column(
      ...
      children: [
        Container(
          width: 44,
          height: 44,
          decoration: const BoxDecoration(gradient: AppColors.goldFill, shape: BoxShape.circle),
          alignment: Alignment.center,
          child: Text(
            '${index + 1}',
...
class _SiteLinkState extends State<SiteLink> {
  ...
  @override
  Widget build(BuildContext context) {
    final color = widget.color ?? AppColors.goldDeep;
    return MouseRegion(
